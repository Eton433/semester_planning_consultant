<template>
      ...
      <div class="container">
        ...
        <div class="records-section">
          <div class="section-header">
            <h3 class="section-title">📋 已填成績</h3>
          ...
          <div v-else class="records-grid">
            <div v-for="record in performanceRecords" :key="record.course_id" class="record-card">
              <div class="course-info">
                <div class="course-code">{{ record.course_id }}</div>
                <div class="course-name">{{ record.course_name }}</div>
              </div>
              <div class="score-section">
                <div class="score-value" :class="getScoreClass(record.course_score)">
                  {{ record.course_score }}
                </div>
                <div class="score-label">分</div>
              </div>
              <div class="grade-badge" :class="getGradeClass(record.course_score)">
                {{ getGradeLetter(record.course_score) }}
              </div>
            </div>
          </div>
        </div>
  ...
  const submitPerformance = async () => {
    if (!selectedCourse.value || !score.value) {
      successMessage.value = '❌ 請填寫完整資訊'
      setTimeout(() => (successMessage.value = ''), 3000)
      return
    }
  
    if (score.value < 0 || score.value > 100) {
      successMessage.value = '❌ 分數必須在 0-100 之間'
      setTimeout(() => (successMessage.value = ''), 3000)
      return
    }
    ...
    try {
      ...
      successMessage.value = '❌ 成績上傳失敗，請稍後再試'
      setTimeout(() => (successMessage.value = ''), 3000)
    }
  }
